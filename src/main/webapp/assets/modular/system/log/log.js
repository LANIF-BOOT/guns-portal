layui.use(['layer', 'table', 'ax', 'laydate'], function () {
    ...
    LoginLog.logDetail = function (param) {
        var ajax = new $ax(Feng.ctxPath + "/manage/log/detail/" + param.operationLogId, function (data) {
    ...
    LoginLog.cleanLog = function () {
        Feng.confirm("是否清空所有日志?", function () {
            var ajax = new $ax(Feng.ctxPath + "/manage/log/delLog", function (data) {
                Feng.success("清空日志成功!");
                LoginLog.search();
            }, function (data) {
                Feng.error("清空日志失败!");
    ...
    var tableResult = table.render({
        ...
        url: Feng.ctxPath + '/manage/log/list',
        page: true,
        height: "full-98",
        cellMinWidth: 100,
