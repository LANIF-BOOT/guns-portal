layui.use(['layer', 'form', 'table', 'ztree', 'laydate', 'admin', 'ax'], function () {
    ...
    MgrUser.openAddUser = function () {
        ...
        top.layui.admin.open({
            type: 2,
            title: '添加用户',
            content: Feng.ctxPath + '/manage/mgr/user_add',
            end: function () {
    ...
    MgrUser.onEditUser = function (data) {
        ...
        top.layui.admin.open({
            type: 2,
            title: '编辑用户',
            content: Feng.ctxPath + '/manage/mgr/user_edit?userId=' + data.userId,
            end: function () {
                admin.getTempData('formOk') && table.reload(MgrUser.tableId);
            }
        });
    ...
    MgrUser.onDeleteUser = function (data) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/manage/mgr/delete", function () {
    ...
    MgrUser.roleAssign = function (data) {
        layer.open({
            ...
            title: '角色分配',
            area: ['300px', '400px'],
            content: Feng.ctxPath + '/manage/mgr/role_assign?userId=' + data.userId,
            end: function () {
                table.reload(MgrUser.tableId);
            }
        });
    ...
    MgrUser.resetPassword = function (data) {
        Feng.confirm("是否重置密码为111111 ?", function () {
            var ajax = new $ax(Feng.ctxPath + "/manage/mgr/reset", function (data) {
                Feng.success("重置密码成功!");
            }, function (data) {
                Feng.error("重置密码失败!");
            });
    ...
    MgrUser.changeUserStatus = function (userId, checked) {
        if (checked) {
            var ajax = new $ax(Feng.ctxPath + "/manage/mgr/unfreeze", function (data) {
                Feng.success("解除冻结成功!");
            }, function (data) {
                Feng.error("解除冻结失败!");
                table.reload(MgrUser.tableId);
            ...
            var ajax = new $ax(Feng.ctxPath + "/manage/mgr/freeze", function (data) {
                Feng.success("冻结成功!");
            }, function (data) {
    ...
    var tableResult = table.render({
        ...
        url: Feng.ctxPath + '/manage/mgr/list',
        page: true,
        height: "full-98",
        cellMinWidth: 100,
    ...
    //初始化左侧部门树
    var ztree = new $ZTree("deptTree", "/manage/dept/tree");
