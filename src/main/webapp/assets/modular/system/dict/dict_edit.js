...
layui.use(['form', 'ax'], function () {
    ...
    //获取详情信息，填充表单
    var ajax = new $ax(Feng.ctxPath + "/manage/dict/detail?dictId=" + Feng.getUrlParam("dictId"));
    var result = ajax.start();
    form.val('dictForm', result.data);

    //表单提交事件
    form.on('submit(btnSubmit)', function (data) {
        var ajax = new $ax(Feng.ctxPath + "/manage/dict/editItem", function (data) {
            Feng.success("更新成功！");
            window.location.href = Feng.ctxPath + "/manage/dict?dictTypeId=" + $("#dictTypeId").val();
        }, function (data) {
    ...
    $('#parentName').click(function () {
        ...
        var treeUrl = encodeURIComponent("/manage/dict/ztree?dictTypeId=" + $("#dictTypeId").val() + "&dictId=" + $("#dictId").val());
