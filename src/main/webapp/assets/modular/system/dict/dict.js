layui.use(['table', 'ax', 'treetable'], function () {
    ...
    Dict.openAddDlg = function () {
        window.location.href = Feng.ctxPath + '/manage/dict/add?dictTypeId=' + $("#dictTypeId").val();
    };
    ...
    Dict.openEditDlg = function (data) {
        window.location.href = Feng.ctxPath + '/manage/dict/edit?dictId=' + data.dictId;
    };
    ...
    Dict.onDeleteItem = function (data) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/manage/dict/delete", function (data) {
                Feng.success("删除成功!");
                Dict.search();
            }, function (data) {
    ...
    Dict.initTable = function (dictId, data) {
        return treetable.render({
            elem: '#' + dictId,
            url: Feng.ctxPath + '/manage/dict/list?dictTypeId=' + $("#dictTypeId").val(),
            where: data,
            height: "full-98",
            cellMinWidth: 100,
    ...
    $('#btnBack').click(function () {
        window.location.href = Feng.ctxPath + "/manage/dictType";
    });
